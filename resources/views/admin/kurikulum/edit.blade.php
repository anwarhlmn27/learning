...
@section('title', __('Edit Curriculum'))

@section('header_left')
    <h1 style="font-size: 1.25rem; font-weight: 700; margin: 0;">{{ __('Edit Curriculum') }}</h1>
@endsection
...
<div class="card" style="max-width: 900px; margin: 0 auto;">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('kurikulum.update', $kurikulum->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <h3 style="font-size: 1rem; font-weight: 600; margin-top: 2rem; border-bottom: 1px solid #e5e7eb; padding-bottom: 0.5rem;">{{ __('Document Attachments') }}</h3>
            <p style="font-size: 0.75rem; color: var(--text-muted);">{{ __('Leave empty to keep existing files.') }}</p>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-top: 1rem;">
                <div class="form-group">
                    <label class="form-label">{{ __('Berita Acara FGD') }} @if($kurikulum->berita_acara_fgd) <span style="color: var(--success);">✓</span> @endif</label>
                    <input type="file" name="berita_acara_fgd" class="form-control" accept=".pdf">
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('Daftar Hadir') }} @if($kurikulum->daftar_hadir) <span style="color: var(--success);">✓</span> @endif</label>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="form-group">
                    <label class="form-label">{{ __('Notulensi Diskusi') }} @if($kurikulum->notulensi_diskusi) <span style="color: var(--success);">✓</span> @endif</label>
                    <input type="file" name="notulensi_diskusi" class="form-control" accept=".pdf">
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('Laporan Penyusunan') }} @if($kurikulum->laporan_penyusunan) <span style="color: var(--success);">✓</span> @endif</label>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="form-group">
                    <label class="form-label">{{ __('Laporan Sosialisasi') }} @if($kurikulum->laporan_sosialisasi) <span style="color: var(--success);">✓</span> @endif</label>
                    <input type="file" name="laporan_sosialisasi" class="form-control" accept=".pdf">
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('Dokumentasi') }} @if($kurikulum->dokumentasi) <span style="color: var(--success);">✓</span> @endif</label>
