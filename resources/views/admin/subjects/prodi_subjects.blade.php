...
@section('header_left')
    <h1 style="font-size: 1.25rem; font-weight: 700; margin: 0;">Subjects: {{ $prodi->nama_prodi }}</h1>
@endsection

@section('content')
<div style="margin-bottom: 1rem;">
    <a href="{{ route('subjects.index') }}" class="btn btn-secondary" style="padding: 0.25rem 0.5rem; font-size: 0.75rem; background: #f3f4f6; color: #1f2937; text-decoration: none; border-radius: 4px; border: 1px solid #e5e7eb;">&larr; Back to Prodi List</a>
</div>

<div class="card">
    <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.5rem;">
        <span>List of Subjects ({{ $subjects->count() }})</span>
        <div style="display: flex; gap: 0.5rem;">
            ...
            <a href="{{ route('subjects.create') }}?prodi_id={{ $prodi->id }}" class="btn btn-primary">Add New Subject</a>
        </div>
    </div>
        ...
        <div style="overflow-x: auto;">
            <table>
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="text-align: center; color: var(--text-muted); padding: 2rem;">No subjects found for this Prodi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
