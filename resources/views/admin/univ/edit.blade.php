        ...
        <form action="{{ route('univ.update', $univ->id) }}" method="POST" enctype="multipart/form-data" id="univForm">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="text" name="kode_univ" class="form-control" required value="{{ old('kode_univ', $univ->kode_univ) }}">
                </div>
                <div class="form-group">
                    <label class="form-label">University Name</label>
                    <input type="text" name="nama_univ" class="form-control" required value="{{ old('nama_univ', $univ->nama_univ) }}">
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="text" name="nama_pimpinan" class="form-control" required value="{{ old('nama_pimpinan', $univ->nama_pimpinan) }}">
                </div>
            </div>

            <div class="form-group">
                <label class="form-label">Full Address</label>
                <textarea name="address" class="form-control" rows="3" required>{{ old('address', $univ->address) }}</textarea>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="email" name="email" class="form-control" required value="{{ old('email', $univ->email) }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Website (URL)</label>
                    <input type="url" name="website" class="form-control" required value="{{ old('website', $univ->website) }}">
                </div>
            </div>
